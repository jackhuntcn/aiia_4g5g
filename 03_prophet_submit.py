...
def run_prophet(data, metric, user, min_clip_ratio=0.8, max_clip_ratio=1.2):
    '''
    输入: 
    ...
    if df['y'].nunique() < 2:                          # 存在数据缺失全为 0 的情况
    ...
    # target boxcox 转换
    df['y'] += 1e-3
    xt, fitted_lambda = stats.boxcox(df['y'])
    df['y'] = xt
    ...
    ret = special.inv_boxcox(ret, fitted_lambda) - 1e-3
    ret = np.clip(ret, min_clip_ratio*min_val, max_clip_ratio*max_val, out=None)
...
def batch_run_prophet(filename):
    ...
    for idx, row in tqdm(sub_df.iterrows()):
        user = row['UserLabel']
        if metric in ['PDCPDL', 'PDCPUL']:
            pred = run_prophet(data, metric, user, 
                               min_clip_ratio=0.5, max_clip_ratio=1.5)
        else:
            pred = run_prophet(data, metric, user, 
                               min_clip_ratio=0.8, max_clip_ratio=1.2)
