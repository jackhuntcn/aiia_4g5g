...
sub_dir = './backup/result_data_xgboost'
...
opt_4g = pd.read_csv('./opt_data/4g_opt.csv')
opt_5g = pd.read_csv('./opt_data/5g_opt.csv')
...
def train_model(fn, train_epochs=100):
    ...
    not_used = ['UserLabel', 'TimeStamp', 'city']
    if category == '5g':
    ...
    model = xgb.XGBRegressor(max_leaves=31,
                             max_depth=5,
                             learning_rate=0.1,
                             n_estimators=train_epochs,
                             subsample=0.8,
    ...
    # 保存
    joblib.dump(xgb_model, f'./models/xgb_model_{real_fn}.pickle')
    print('used time:', time.time() - start_time, 'seconds')
    
    return xgb_model, feature_names, lbe
...
for idx, fn in enumerate(test_files):
    ...
    xgb_model, feature_names, lbe = train_model(fn, train_epochs=3000)
    df_importance = pd.DataFrame({
        'column': feature_names,
        'importance': xgb_model.feature_importances_,
    })
    print(df_importance.sort_values('importance', ascending=False))
    sub = batch_predict(fn, lbe, xgb_model, feature_names)
